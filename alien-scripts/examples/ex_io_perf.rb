...
Here we read the digital input port a number of times and report the average 
...
# create a reader
	...
	if r.open(ipaddress) #connect and login w/defaults
		...
		puts "Total time: #{t.elapsed} sec \t Time per transaction: #{1000*t.elapsed.to_f/num_meas} msec"
		puts "----------------------------------"
