...
Sometimes controlling the details of the way the reader broadcasts 
commands to the tags can be important for optimum performance. 
(Particularly in multi-reader environments.) 
Here we show how to control the output power of the reader, the antennas used, 
and the RF modulation scheme.
...
# connect to the reader
	...
	# our reader returns power in dBm *10 
		puts 'Current RF Power: ' + ( old_rf_level.to_f / 10 ).to_s + 'dBm'
	...
	# set the reader to use all the connected antennas
		...
		r.rflevel='300'
		puts 'RF Level: ' + (r.rflevel.to_f / 10).to_s + 'dBm'
